...
local Spotify_Connections = {
	PauseButton = nil,
	RevertButton = nil,
	SkipButton = nil
}
run(function()
	local SpotifyIntegration = {Enabled = false}
	local SpotifyControlls = {Enabled = false}
	local Spotify = GuiLibrary.CreateCustomWindow({
		Name = "SpotifyPlayer",
		Icon = "vape/assets/TargetInfoIcon1.png",
		IconSize = 16
	})
	...
	shared.tweentask = nil
	if (not isfolder(baseDirectory..'Spotify')) then makefolder(baseDirectory.."Spotify") end
	...
	--[[local Assets = {
		Play = "rbxassetid://101719148876396",
		Pause = "rbxassetid://94476881064430",
		Next = "rbxassetid://118222964071614",
		Revert = "rbxassetid://82730819818092"
	}--]]
	local Spotify_Assets = {
		Play = "rbxassetid://112034695434428",
		Pause = "rbxassetid://76205181023196",
		Next = "rbxassetid://102974955942372",
		Revert = "rbxassetid://76522860778768"
	}
	local Spotify_API = {
		accessToken = "",
		refreshToken = ""
	}
	function Spotify_API:registerTokens(tokenTbl)
		Spotify_API.accessToken = "Bearer "..tokenTbl.accessToken
		Spotify_API.refreshToken = tokenTbl.refreshToken
	end
	function Spotify_API:fetchData()
		return request({
			Method = "GET",
			Url = "https://api.spotify.com/v1/me/player",
			Headers = {
				["Content-Type"] = "application/json",
				["Authorization"] = Spotify_API.accessToken
			}
		})
	end
	function Spotify_API:downloadSongImage(songName, albumImageUrl)
		local imagePath = baseDirectory.."Spotify/" .. tostring(songName) .. ".png"
		task.spawn(function()
			local textlabel = Instance.new("TextLabel")
			textlabel.Size = UDim2.new(1, 0, 0, 36)
			textlabel.Text = "Downloading "..imagePath
			textlabel.BackgroundTransparency = 1
			textlabel.TextStrokeTransparency = 0
	...
	function Spotify_API:fetchSongImage(songName, albumImageUrl)
		if not isfolder(baseDirectory.."Spotify") then makefolder(baseDirectory.."Spotify") end
		local imagePath = baseDirectory.."Spotify/" .. tostring(songName) .. ".png"
		if (not isfile(imagePath)) then
			local StatusCode, Body = Spotify_API:downloadSongImage(songName, albumImageUrl)
			if StatusCode ~= 200 then errorNotification("SpoitifyPlayer", "Error downloading songImage for song: "..tostring(songName).."!", 3) else
	...
	function Spotify_API:refresh()
		local refreshOld = Spotify_API.accessToken
		local res = request({
			Method = "GET",
			Url = "https://spotify.vapevoidware.xyz/api/spotify/refresh?refresh_token="..tostring(Spotify_API.refreshToken),
			Headers = {
				["Content-Type"] = "application/json"
			}
		})
		if res.StatusCode ~= 200 then
			local suc, realErr = pcall(function() return HttpService:JSONDecode(res.Body).error end)
			if (not suc) then realErr = "Unknown error!" end
			realErr = tostring(realErr)
			errorNotification("SpotifyPlayer - Spotify_API:refresh()", realErr, 3)
		else
			Spotify_API:registerTokens({
				accessToken = HttpService:JSONDecode(res.Body).accessToken,
				refreshToken = Spotify_API.refreshToken
			})
		end
		local refreshNew = Spotify_API.accessToken
		if shared.VoidDev and shared.SpotifyDebug then
			print("[refresh - old]: "..tostring(refreshOld))
			print("[refresh - new]: "..tostring(refreshNew))
		end
	end
	function Spotify_API:send(method, endpoint)
		local baseURL = "https://api.spotify.com/v1/me/player"
		local res = request({
			Method = method,
	...
	local refreshTokensLoop
	local refreshSpotifyGUILoop
	local isPaused = false
	local function createOrUpdatePlayer(jsonData, isLightMode)
		if shared.currenttween then pcall(function() shared.currenttween:Cancel() end) end
		if shared.tweentask then pcall(function() task.cancel(shared.tweentask) end) end
		--for i,v in pairs(Spotify_Connections) do pcall(function() v:Disconnect() end) end
		local existingBackground = Spotify.GetCustomChildren():FindFirstChild("Background")
		local songData = HttpService:JSONDecode(jsonData)
		local songName = songData.item.name
		local artistName = songData.item.artists[1].name
		local albumImageUrl = songData.item.album.images[1].url
		local songDuration = songData.item.duration_ms / 1000
		local currentProgress = songData.progress_ms / 1000
		...
		local background, songImage, songNameLabel, artistLabel, progressBar, progressIndicator, timeLabel, pauseButton, revertButton, skipButton
	
		if existingBackground then
			background = existingBackground
			songImage = background.SongImage
			songNameLabel = background.SongNameLabel
			artistLabel = background.ArtistLabel
			progressBar = background.ProgressBar
			progressIndicator = progressBar.ProgressIndicator
			timeLabel = background.TimeLabel
			pauseButton = background.PauseButton
			revertButton = background.RevertButton
			skipButton = background.SkipButton

			pauseButton.Visible = SpotifyControlls.Enabled
			revertButton.Visible = SpotifyControlls.Enabled
			skipButton.Visible = SpotifyControlls.Enabled
	
			songNameLabel.Text = songName
			artistLabel.Text = "by " .. artistName
	
			songImage.Image = Spotify_API:fetchSongImage(songName, albumImageUrl)
	
			local imageTween = TweenService:Create(songImage, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0})
			imageTween:Play()
	
			progressIndicator.Size = UDim2.new(currentProgress / songDuration, 0, 1, 0)
			--[[local function updateProgressBar()
				local progressRatio = currentProgress / songDuration
				local tweenInfo = TweenInfo.new(songDuration - currentProgress, Enum.EasingStyle.Linear)
				local tween = TweenService:Create(progressIndicator, tweenInfo, {Size = UDim2.new(1, 0, 1, 0)})
				shared.currenttween = tween
		
				tween:Play()
		
				task.spawn(function()
					for i = currentProgress, songDuration do
						timeLabel.Text = formatTime(i) .. " / " .. formatTime(songDuration)
						wait(1)
					end
				end)
			end--]]
			...
			background = Instance.new("Frame", Spotify.GetCustomChildren())
			background.Name = "Background"
			background.Position = UDim2.new(0, 10, 0, 10)
			background.Size = UDim2.new(0, 200, 0, 260)
			local color = GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSliderColor"].Api
			background.BackgroundColor3 = Color3.fromHSV(color.Hue, color.Sat, color.Value)

			local gradient = Instance.new("UIGradient", background)
			gradient.Color = ColorSequence.new{
				ColorSequenceKeypoint.new(0, Color3.fromHSV(color.Hue, color.Sat, color.Value)),
			...
			task.spawn(function()
				repeat 
					...
					if shared.RiseMode then
						gradient.Color = ColorSequence.new{
							ColorSequenceKeypoint.new(0, GuiLibrary.GUICoreColor),
							ColorSequenceKeypoint.new(1, backgroundColor)
						}
					end
				until (not SpotifyIntegration.Enabled)
			end)
			...
			songImage = Instance.new("ImageLabel", background)
			songImage.Name = "SongImage"
			songImage.Position = UDim2.new(0.13, 0, 0.1, 0)
			songImage.Size = UDim2.new(0, 150, 0, 150)
			songImage.BackgroundTransparency = 1
	
			songImage.Image = Spotify_API:fetchSongImage(songName, albumImageUrl)
			local songImageCorner = Instance.new("UICorner", songImage)
			songImageCorner.CornerRadius = UDim.new(0, 15)
	
			songImage.ImageTransparency = 1
			local imageTween = TweenService:Create(songImage, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0})
			imageTween:Play()
	
			songNameLabel = Instance.new("TextLabel", background)
			songNameLabel.Name = "SongNameLabel"
			songNameLabel.Position = UDim2.new(0.05, 0, 0.7, 0)
			songNameLabel.Size = UDim2.new(0, 180, 0, 20)
			songNameLabel.Text = songName
			songNameLabel.TextScaled = true
			songNameLabel.BackgroundTransparency = 0.5
			songNameLabel.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
			songNameLabel.TextColor3 = textColor
			songNameLabel.Font = Enum.Font.SourceSansBold
	
			local nameCorner = Instance.new("UICorner", songNameLabel)
			nameCorner.CornerRadius = UDim.new(0, 10)
	
			artistLabel = Instance.new("TextLabel", background)
			artistLabel.Name = "ArtistLabel"
			artistLabel.Position = UDim2.new(0.05, 0, 0.8, 0)
			artistLabel.Size = UDim2.new(0, 180, 0, 20)
			artistLabel.Text = "by " .. artistName
			artistLabel.TextScaled = true
			artistLabel.BackgroundTransparency = 0.5
			artistLabel.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
			artistLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
			artistLabel.Font = Enum.Font.SourceSans
	
			local artistCorner = Instance.new("UICorner", artistLabel)
			artistCorner.CornerRadius = UDim.new(0, 10)
	
			progressBar = Instance.new("Frame", background)
			progressBar.Name = "ProgressBar"
			progressBar.Position = UDim2.new(0.05, 0, 0.92, 0)
			progressBar.Size = UDim2.new(0, 180, 0, 8)
			progressBar.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
	
			local progressBarCorner = Instance.new("UICorner", progressBar)
			progressBarCorner.CornerRadius = UDim.new(1, 0)
	
			progressIndicator = Instance.new("Frame", progressBar)
			progressIndicator.Name = "ProgressIndicator"
			progressIndicator.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
			progressIndicator.Size = UDim2.new(currentProgress / songDuration, 0, 1, 0)
	
			local progressIndicatorCorner = Instance.new("UICorner", progressIndicator)
			progressIndicatorCorner.CornerRadius = UDim.new(1, 0)
	
			timeLabel = Instance.new("TextLabel", background)
			timeLabel.Name = "TimeLabel"
			timeLabel.Position = UDim2.new(0.6, 0, 0.89, 0)
			timeLabel.Size = UDim2.new(0, 70, 0, 20)
			...
			pauseButton = Instance.new("ImageButton", background)
			pauseButton.Name = "PauseButton"
			pauseButton.Position = UDim2.new(0.4, 0, 0.95, 0)
			pauseButton.Size = UDim2.new(0, 40, 0, 40)
			pauseButton.BackgroundTransparency = 1
			pauseButton.Image = Spotify_Assets.Pause
			pauseButton.Visible = SpotifyControlls.Enabled
			local con1 = pauseButton.MouseButton1Click:Connect(function()
				local res = Spotify_API:send("PUT", isPaused and "/play" or "/pause")
				if res.StatusCode == 200 then
					isPaused = (not isPaused)
					pauseButton.Image = isPaused and Spotify_Assets.Play or Spotify_Assets.Pause
					InfoNotification("SpotifyPlayer - "..(isPaused and "Pause" or "Resume").." Song", "Success!", 1.5)
				else
					if string.find(string.lower(encode(res)), "premium_required") then
						errorNotification("SpotifyPlayer - "..(isPaused and "Resume" or "Pause").." Song", "Error! Spotify Premium required!", 3)
					else
						errorNotification("SpotifyPlayer - "..(isPaused and "Resume" or "Pause").." Song", "Error! ["..tostring(res.StatusCode).."]", 3)
					end
					warn("[SpotifyPlayer - "..(isPaused and "Play" or "Stop").." Song]", encode(res))
				end
				print("Pause/Play Button")
			end)	
			Spotify_Connections.PauseButton = con1
	
			revertButton = Instance.new("ImageButton", background)
			revertButton.Name = "RevertButton"
			revertButton.Position = UDim2.new(0.1, 0, 0.95, 0)
			revertButton.Size = UDim2.new(0, 40, 0, 40)
			revertButton.BackgroundTransparency = 1
			revertButton.Image = Spotify_Assets.Revert
			revertButton.Visible = SpotifyControlls.Enabled
			local con2 = revertButton.MouseButton1Click:Connect(function()
				local res = Spotify_API:send("POST", "/previous")
				if res.StatusCode == 200 then
					InfoNotification("SpotifyPlayer - Previous Song", "Success!", 1.5)
				else
					if string.find(string.lower(encode(res)), "premium_required") then
						errorNotification("SpotifyPlayer - Previous Song", "Error! Spotify Premium required!", 3)
					else
						errorNotification("SpotifyPlayer - Previous Song", "Error! ["..tostring(res.StatusCode).."]", 3)
					end
					warn("[SpotifyPlayer - Previous Song]", encode(res))
				end
				print("RevertButton")
			end)
			Spotify_Connections.RevertButton = con2
	
			skipButton = Instance.new("ImageButton", background)
			skipButton.Name = "SkipButton"
			skipButton.Position = UDim2.new(0.7, 0, 0.95, 0)
			skipButton.Size = UDim2.new(0, 40, 0, 40)
			skipButton.BackgroundTransparency = 1
			skipButton.Image = Spotify_Assets.Next
			skipButton.Visible = SpotifyControlls.Enabled
			local con3 = skipButton.MouseButton1Click:Connect(function()
				local res = Spotify_API:send("POST", "/next")
				if res.StatusCode == 200 then
					InfoNotification("SpotifyPlayer - Skip Song", "Success!", 1.5)
				else
					if string.find(string.lower(encode(res)), "premium_required") then
						errorNotification("SpotifyPlayer - Skip Song", "Error! Spotify Premium required!", 3)
					else
						errorNotification("SpotifyPlayer - Skip Song", "Error! ["..tostring(res.StatusCode).."]", 3)
					end
					warn("[SpotifyPlayer - Skip Song]", encode(res))
				end
				print("SkipButton")
			end)
			Spotify_Connections.SkipButton = con3
		end
		...
		local function updateProgressBar()
			...
			shared.tweentask = task.spawn(function()
				for i = currentProgress, songDuration do
					timeLabel.Text = formatTime(i) .. " / " .. formatTime(songDuration)
					wait(1)
				end
	...
	local function RequestNewToken()
		InfoNotification("SpotifyPlayer", "Please head over to spotify.vapevoidware.xyz and get a new token!     ", 3)
		setclipboard("spotify.vapevoidware.xyz")
		InfoNotification("SpotifyPlayer", "Set to clipboard spotify.vapevoidware.xyz!     ", 3)
	end
	local SpotifyToken = {Value = isfile(baseDirectory.."Spotify/spotifytoken.txt") and readfile(baseDirectory.."Spotify/spotifytoken.txt") or ""}
	VoidwareFunctions.GlobaliseObject("spotifytoken", SpotifyToken.Value)
	SpotifyIntegration = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "SpotifyPlayer",
		Function = function(call)
			if call then
				refreshTokensLoop = coroutine.create(function()
					repeat
						task.wait(1000)
						Spotify_API:refresh()
					until (not SpotifyIntegration.Enabled)
				end)
				refreshSpotifyGUILoop = coroutine.create(function()
					repeat
						task.wait(1)
						local res = Spotify_API:fetchData()
						--print(encode(res))
						if res.StatusCode ~= 200 then errorNotification("SpotifyPlayer", "Invalid token! [2-"..tostring(res.StatusCode).."]", 3); RequestNewToken(); SpotifyIntegration.ToggleButton(false); return else
							createOrUpdatePlayer(res.Body, false)
							Spotify.SetVisible(true)
						end
					until (not SpotifyIntegration.Enabled)
				end)
				if SpotifyToken.Value == "" then errorNotification("SpotifyPlayer", "Please specify your spotify token in the textbox!", 3); RequestNewToken(); SpotifyIntegration.ToggleButton(false); return else
					local suc, tokens = pcall(function() return HttpService:JSONDecode(VoidwareFunctions.Base64.decode(SpotifyToken.Value)) end)
					if (not suc) then errorNotification("SpotifyPlayer", "Invalid token! [1]", 3); SpotifyIntegration.ToggleButton(false); return end
					if (not tokens.accessToken) or (not tokens.refreshToken) then errorNotification("SpotifyPlayer", "Invalid token! [2]", 3); RequestNewToken(); SpotifyIntegration.ToggleButton(false); return else
						Spotify_API:registerTokens(tokens)
						if shared.VoidDev then setclipboard(tostring(Spotify_API.accessToken)) end
						Spotify_API:refresh()
						local res = Spotify_API:fetchData()
						--print(encode(res))
						if res.StatusCode ~= 200 then 
							if res.StatusCode == 204 then
								errorNotification("SpotifyPlayer", "Please play a song.", 3);
							else errorNotification("SpotifyPlayer", "Invalid token! [2-"..tostring(res.StatusCode).."]", 3); RequestNewToken(); end
							SpotifyIntegration.ToggleButton(false); 
							return else
							createOrUpdatePlayer(res.Body, false)
							Spotify.SetVisible(true)
							coroutine.resume(refreshTokensLoop)
							coroutine.resume(refreshSpotifyGUILoop)
						end
					end
				end
			else
				if Spotify.GetCustomChildren():FindFirstChild("Background") then Spotify.GetCustomChildren():FindFirstChild("Background"):Destroy() end
				Spotify.SetVisible(false)
				coroutine.close(refreshTokensLoop)
				coroutine.close(refreshSpotifyGUILoop)
			end
		end
	})
	SpotifyIntegration.Restart = function()
		if SpotifyIntegration.Enabled then SpotifyIntegration.ToggleButton(false); SpotifyIntegration.ToggleButton(false) end
	end
	SpotifyToken = SpotifyIntegration.CreateTextBox({
		Name = "Spotify Token",
		TempText = "Enter here your spotify token",
		Default = SpotifyToken.Value:gsub("%s+", ""),
		FocusLost = function(inp)
			if inp then
				VoidwareFunctions.GlobaliseObject("spotifytoken", SpotifyToken.Value:gsub("%s+", ""))
	            writefile(baseDirectory.."Spotify/spotifytoken.txt", SpotifyToken.Value:gsub("%s+", ""))
	            SpotifyIntegration.Restart()
			end
        end
	})
	SpotifyControlls = SpotifyIntegration.CreateToggle({
		Name = "SpotifyControlls",
		Function = SpotifyIntegration.Restart(),
		Default = false
	})
end)